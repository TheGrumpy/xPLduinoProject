...
//Sve
void RCSwitch::enableReceivePCINT(int interrupt) {
  this->nReceiverInterrupt = interrupt;
  PCinterrupt= interrupt;
  if (this->nReceiverInterrupt != -1) {
  	this->enableReceive();
    RCSwitch::nReceivedValue = NULL;
    RCSwitch::nReceivedBitlength = NULL;
     
if (this->nReceiverInterrupt < 8) {
    PCICR  |= ( 1 << PCIE0 );  // Listen for PCINT0 to PCINT7
    PCMSK0  |= ( 1 << (this->nReceiverInterrupt));  // Listen for PCINTx
}else if (this->nReceiverInterrupt < 16) {
    PCICR  |= ( 1 << PCIE1 );  // Listen for PCINT8 to PCINT15
	PCMSK1  |= ( 1 << ((this->nReceiverInterrupt)-8));  // Listen for PCINTx
}else if (this->nReceiverInterrupt < 24) {
    PCICR  |= ( 1 << PCIE2 );  // Listen for PCINT16 to PCINT23
	PCMSK2  |= ( 1 << ((this->nReceiverInterrupt)-16));  // Listen for PCINTx
}else if (this->nReceiverInterrupt < 31) {
    PCICR  |= ( 1 << PCIE3 );  // Listen for PCINT24 to PCINT31
	PCMSK3  |= ( 1 << ((this->nReceiverInterrupt)-24));  // Listen for PCINTx
}
    //IN0_PORT |= _BV(IN0_BIT); // turn on internal pull-up resistor
