...
#define TEMPERATURE_CORE_VERSION 3.1
//------ Version 2.0 ------------
...
//------ Version 3.1 ------------
// Ajout de la configuration en fonction de la précision demandée pour les sondes DS18B20
...
void Temperature::configurePres()
{
#ifdef DEBUG_TEMP_OW
         Serial.print(F("Reconfiguration in Progress ->>>>"));
#endif
// configuration need to be send to device according to the precision
    ds.wireReset();
    ds.romMatch(addr);
    ds.wireWrite(WriteScratchPad);
    
    if (IsArrondi()){
      ds.wireWrite(0x00);ds.wireWrite(0x00);ds.wireWrite(0x1F);          // configuration pour arrondi  à 0,5 près (precision 9 bits 0,5°C)
    } else {
      ds.wireWrite(0x00);ds.wireWrite(0x00);ds.wireWrite(0x7F);          // configuration pour arrondi  à 0,1 près (precision 12 bits 0,0625°C)
    }
// configuration need to be store in the DS18B20
    ds.wireReset();
    ds.romMatch(addr);
    ds.wireWrite(CopyScratchPad);  
    
// reset reconfiguration flag
postConfOk();
#ifdef DEBUG_TEMP_OW
         Serial.println(F(">>>--  Reconfiguration finish"));
#endif
}
...
#endif
if (!IsConf() && addr[0]==DS18B20MODEL){configurePres();}  // If the configuration is not correct the configuration is updated inside the DS18B20
// Ask the device to convert the temperature
    ds.wireReset();
    ds.romMatch(addr);
    ...
    case DS1822MODEL:
      ...
      case TEMP_12_BIT:
       if (IsArrondi()){
         postConfNok();
#ifdef DEBUG_TEMP_OW
         Serial.println(F("Device is not well configured  - reconfiguration is request"));
#endif
       }
#ifdef DEBUG_TEMP_OW
        Serial.println(F("Precision 12Bit  - 0,0625C "));
#endif
        tempC= (float)rawTemperature * 0.0625;
        break;
      case TEMP_11_BIT:
#ifdef DEBUG_TEMP_OW
        Serial.println(F("Precision 11Bit  - 0,125C "));
#endif
        tempC= (float)(rawTemperature >> 1) * 0.125;
        break;
      case TEMP_10_BIT:
#ifdef DEBUG_TEMP_OW
        Serial.println(F("Precision 10Bit  - 0,25C "));
#endif
...
#ifdef DEBUG_TEMP_OW
        Serial.println(F("Precision 9Bit  - 0,5C "));
#endif
...
// function to check is the configuration is OK
uint8_t  Temperature::IsConf(){
  return bitRead(Temp_status, Addr_confOK);
}
...
// function to make the configuration is ok
void  Temperature::postConfOk(){
  bitSet(Temp_status, Addr_confOK);
}
// function to make the configuration is not ok
void  Temperature::postConfNok(){
  bitClear(Temp_status, Addr_confOK);
}
