...
/// probleme potentiel: si la tempo est dans une equation et qu'elle n'est pas �valu�, ca va merder au prochain usage
/// il faudrait alors mettre � jour le soft pour �valuer systematiquement les tempos, meme si pas appel�
/// on pourrait mettre un flag � vrai en interne et l'update se ferait hors scenarios
...
void Timer::update_tof()
{
    ...
    if(bitRead(status, Q) &! bitRead(status, INPUT)){
        ...
        if(current){ // on continue de compter si on a pas d�j� atteint la consigne
            switch(base){
        ...
        }else{ // on a termin� de compter

            //~ if(bitRead(status, Q)) // si on ne compte plus et qu'on a pas encore bascul� la sortie on bascule le one shot
            bitSet(status, Q_ONS);
    ...
    bitClear(status, INPUT); // on efface le flag d'entr�e, il doit normalement �tre remis � vrai dans le scenario. Si ce n'est pas le cas, la tempo est remise � z�ro.
...
void Timer::update_ton()
{
    ...
    if(bitRead(status, INPUT)){
        ...
        if(current < setpoint){ // on continue de compter si on a pas d�j� atteint la consigne
            switch(base){
